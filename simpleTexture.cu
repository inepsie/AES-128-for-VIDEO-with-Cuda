typedef unsigned char INTTYPE;
#define NBOCTKEY 16
#define NBKEY 11

#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include <math.h>

#ifdef _WIN32
#  define WINDOWS_LEAN_AND_MEAN
#  define NOMINMAX
#  include <windows.h>
#endif

// Includes CUDA
#include <cuda_runtime.h>

// Utilities and timing functions
#include <helper_functions.h>    // includes cuda.h and cuda_runtime_api.h

// CUDA helper functions
#include <helper_cuda.h>         // helper functions for CUDA error check

////////////////////////////////////////////////////////////////////////////////
//! Transform an image using texture lookups
//! @param outputData  output data in global memory
////////////////////////////////////////////////////////////////////////////////

__global__ void aesXor(INTTYPE * a, INTTYPE * k, INTTYPE ind, INTTYPE * b){
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  b[i] = a[i] ^ k[ind * 16 + i % NBOCTKEY];
}

__global__ void sbox(INTTYPE * a, INTTYPE * b){
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  __shared__ INTTYPE sbx[256];
  if(threadIdx.x==0){
  INTTYPE sb[256] = {
      };
  
      for(int i=0 ; i<256 ; ++i){
          sbx[i] = sb[i];
      }
  }
  __syncthreads(); // barrière de synchro
   
  b[i] = sbx[a[i]];
}


__global__ void inv_sbox(INTTYPE * a, INTTYPE *b){
int i = blockIdx.x * blockDim.x + threadIdx.x;
  __shared__ INTTYPE inv_sbx[256];
  if(threadIdx.x==0){
 INTTYPE inv_sb[256] = {};

      for(int i=0 ; i<256 ; ++i){
          inv_sbx[i] = inv_sb[i];
      }
  }
  __syncthreads(); // barrière de synchro
b[i] = inv_sbx[a[i]];
}


__global__ void shiftRows(INTTYPE * a, INTTYPE *b){
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  INTTYPE box[16] = {0, 1, 2, 3, 5, 6, 7, 4, 10, 11, 8, 9, 15, 12, 13, 14};
  b[i] = a[box[i%16] + (i/16) * 16];
}

__global__ void inv_shiftRows(INTTYPE * a, INTTYPE * b){
int i = blockIdx.x * blockDim.x + threadIdx.x;
  const INTTYPE inv_box[16] = {0, 1, 2, 3, 7, 4, 5, 6, 10, 11, 8, 9, 13, 14, 15, 12};
b[i] = a[inv_box[i%16] + (i/16) * 16];
}

__global__ void mixCol(INTTYPE * a, INTTYPE * b){//mat * a
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  const INTTYPE mat[16] = {0x02, 0x03, 0x01, 0x01, 0x01, 0x02, 0x03, 0x01, 0x01, 0x01, 0x02, 0x03, 0x03, 0x01, 0x01, 0x02}; 
  int y = (i%16) / 4;
  int x = (i%16) % 4;
  int h = (i/16)*16 + x;
  b[i] = ((unsigned int)mat[y] * (unsigned int)a[h]) % 256;
  for(int n=1 ; n<4 ; ++n){
      b[i] ^= ((unsigned int)mat[y+n] * (unsigned int)a[h+4*n]) % 256;
  }
}


__global__ void mixColumns(INTTYPE * a, INTTYPE * b){
INTTYPE mat[16] = {1, 2, 0, 0, 0, 1, 2, 0, 0, 0, 1, 2, 2, 0, 0, 1};
  __shared__ INTTYPE multx[3][256];
  if(threadIdx.x==0){
    INTTYPE mult[3][256] = {{0x0,0x1,0x2,0x3,0x4,0x5,0x6,0x7,0x8,0x9,0xa,0xb,0xc,0xd,0xe,
      0xf,},
{},
{}};

      for(int j=0 ; j<3 ; ++j){
          for(int i=0 ; i<256 ; ++i){
              multx[j][i] = mult[j][i];
          }
      }
  }
  __syncthreads(); // barrière de synchro

  int i = blockIdx.x * blockDim.x + threadIdx.x;
  int y = (i%16) >> 2;
  int x = (i%16) % 4;
  //int x = (i%16) & 3;
  int h = (i/16)*16 + x;
  b[i] = multx[mat[4*y]][a[h]];
  for(int n=1 ; n<4 ; ++n){
      b[i] ^= multx[mat[4*y+n]][a[h+4*n]];
  }
}


__global__ void inv_mixColumns(INTTYPE * a, INTTYPE * b){
INTTYPE mat[16] = {0x03, 0x01, 0x02, 0x00, 0x00, 0x03, 0x01, 0x02, 0x02, 0x00, 0x03, 0x01, 0x01, 0x02, 0x00, 0x03};
  __shared__ INTTYPE multx[4][256];
  if(threadIdx.x==0){
    INTTYPE mult[4][256] = {{},
{},
{},
{}};
  
      for(int j=0 ; j<4 ; ++j){
          for(int i=0 ; i<256 ; ++i){
              multx[j][i] = mult[j][i];
          }
      }
  }
  __syncthreads(); // barrière de synchro

  int i = blockIdx.x * blockDim.x + threadIdx.x;
  int y = (i%16) / 4;
  int x = (i%16) % 4;
  //int x = (i%16) & 3;
  int h = (i/16)*16 + x;
  b[i] = multx[mat[4*y]][a[h]];
  for(int n=1 ; n<4 ; ++n){
      b[i] ^= multx[mat[4*y+n]][a[h+4*n]];
  }
}

FILE * ouvrirAb(char * nom){
  FILE * f = NULL;
  f = fopen(nom, "ab");
  assert(f);
  return f;
}

FILE * ouvrirRb(char * nom){
    FILE * f = NULL;
    f = fopen(nom, "rb");
    assert(f);
    return f;
}


__host__ void encryption(INTTYPE ** d_data, INTTYPE ** d_out, INTTYPE * d_key, int threadsPerBlock, int blocksPerGrid){
	for(int i=0 ; i<NBKEY ; ++i){
		aesXor <<<blocksPerGrid, threadsPerBlock>>>(*d_data, d_key, i, *d_out);
		sbox <<<blocksPerGrid, threadsPerBlock>>>(*d_out, *d_data);
		shiftRows <<<blocksPerGrid, threadsPerBlock>>>(*d_data, *d_out);
		mixColumns <<<blocksPerGrid, threadsPerBlock>>>(*d_out, *d_data);
	}
}


__host__ void decryption(INTTYPE ** d_data, INTTYPE ** d_out, INTTYPE * d_key, int threadsPerBlock, int blocksPerGrid){
	for(int i=0 ; i<NBKEY ; ++i){
		inv_mixColumns <<<blocksPerGrid, threadsPerBlock>>>(*d_data, *d_out);
		inv_shiftRows <<<blocksPerGrid, threadsPerBlock>>>(*d_out, *d_data);
		inv_sbox <<<blocksPerGrid, threadsPerBlock>>>(*d_data, *d_out);
		aesXor <<<blocksPerGrid, threadsPerBlock>>>(*d_out, d_key, NBKEY-1-i, *d_data);
	}
}

void cumain(unsigned char * k, unsigned int mode, unsigned char * hData, unsigned int width, unsigned int height, unsigned int Bpp/*En octet*/) {
  static unsigned int size = 0;
  static unsigned char * d_data = NULL;
  static unsigned char * d_out = NULL;
  static int threadsPerBlock, blocksPerGrid;
  static unsigned char * d_key;
  static FILE * f = NULL;
  static char * nom = "data.enc";
  /* ne libérer la mémoire qu'une fois */
  if(!hData) {
    if(d_data) {
      checkCudaErrors(cudaFree(d_data));
      d_data = NULL;
      fclose(f);
    }
    size = 0;
  }
  /* n'allouer qu'une fois */
  if(size == 0) {
    f = ouvrirAb(nom);
    size = Bpp * width * height;
    fwrite(&width, 4, 1, f);
    fwrite(&height, 4, 1, f);
    fwrite(&size, 4, 1, f);
    threadsPerBlock = 1 << 10;
    blocksPerGrid = (size + threadsPerBlock - 1) / threadsPerBlock;
    /* AJOUT */
    // Allocate device memory for result
    checkCudaErrors(cudaMalloc(&d_data, blocksPerGrid * threadsPerBlock * sizeof *d_data));
    checkCudaErrors(cudaMalloc(&d_out, blocksPerGrid * threadsPerBlock * sizeof *d_out));
    checkCudaErrors(cudaMalloc(&d_key, 11 * 16  * sizeof *d_key));

    checkCudaErrors(cudaMemcpy(d_key, k, 11 * 16 * sizeof *d_key, cudaMemcpyHostToDevice));
  }
  checkCudaErrors(cudaMemcpy(d_data, hData, threadsPerBlock * blocksPerGrid * sizeof *d_data, cudaMemcpyHostToDevice));

  checkCudaErrors(cudaDeviceSynchronize());
  StopWatchInterface *timer = NULL;
  sdkCreateTimer(&timer);
  sdkStartTimer(&timer);

  encryption(&d_data, &d_out, d_key, threadsPerBlock, blocksPerGrid);
  decryption(&d_data, &d_out, d_key, threadsPerBlock, blocksPerGrid);



  // Check if kernel execution generated an error
  getLastCudaError("Kernel execution failed");

  checkCudaErrors(cudaDeviceSynchronize());

  // copy result from device to host
  checkCudaErrors(cudaMemcpy(hData,
    d_data,
    size,
    cudaMemcpyDeviceToHost));
  checkCudaErrors(cudaDeviceSynchronize());

  sdkStopTimer(&timer);
  printf("Processing time: %f (ms)\n", sdkGetTimerValue(&timer));
  printf("%.2f Mpixels/sec\n",
	 (Bpp * width * height / (sdkGetTimerValue(&timer) / 1000.0f)) / 1e6);
  sdkDeleteTimer(&timer);
}
